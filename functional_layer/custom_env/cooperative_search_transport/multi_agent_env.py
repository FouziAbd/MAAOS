...
class MultiAgentCooperativeSearchTransportEnv(ParallelEnv):
    ...
    def _handle_drop(self, agent_id: str, rewards: Dict[str, float]):
        ...
        if agent.carrying_object_id is not None:
            ...
            if (obj.is_target and not obj.delivered
                    and obj.position in self.core_env.world.static.delivery_zone):
                obj.delivered = True
    ...
    def _check_object_delivery_rewards(self, rewards: Dict[str, float]):
        # Auto-delivery only for jointly-held cooperative objects. Solo objects are
        # delivered explicitly via DROP (_handle_drop); a solo object merely being
        # carried across the zone must NOT auto-deliver (it is still carried_by an agent).
        for obj in self.core_env.world.objects.values():
            if obj.delivered or obj.carried_by is not None:
                continue
            if not obj.engaged_agents:
                continue
