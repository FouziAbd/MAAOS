...
class TestCase2OptimisticFailure(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        assert [str(c) for c in cls.result.plan] == [
            ...
            "Push(agent_0; box_1; delivery_zone)",
        ], "the frozen 5-step optimistic plan changed identity"
...
class TestCase4MalformedAndUngrounded(unittest.TestCase):
    ...
    def test_malformed_call_converts_to_the_typed_fault_never_a_substitute_skill(self):
        """No P3 parser exists yet, so the MalformedCall is constructed by hand and what this
        case can honestly test is the frozen HANDLING contract. Review WARN 4: asserting
